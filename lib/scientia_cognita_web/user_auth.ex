defmodule ScientiaCognitaWeb.UserAuth do
  use ScientiaCognitaWeb, :verified_routes

  import Plug.Conn
  import Phoenix.Controller

  alias ScientiaCognita.Accounts
  alias ScientiaCognita.Accounts.Scope

  @console_roles ~w(admin owner)

  # Make the remember me cookie valid for 14 days. This should match
  # the session validity setting in UserToken.
  @max_cookie_age_in_days 14
  @remember_me_cookie "_scientia_cognita_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  defp signed_in_path(conn) do
    case conn.assigns[:current_scope] do
      %{user: %{role: role}} when role in @console_roles -> ~p"/console"
      _ -> ~p"/"
    end
  end
  ...
  @doc """
  Plug for routes that require admin or owner role (console access).
  """
  def require_console_user(conn, _opts) do
    user = conn.assigns[:current_scope] && conn.assigns.current_scope.user

    if user && user.role in @console_roles do
      conn
    else
      conn
      |> put_flash(:error, "You don't have permission to access the console.")
      |> maybe_store_return_to()
      |> redirect(to: ~p"/users/log-in")
      |> halt()
    end
  ...
  LiveView on_mount hooks for authentication:
    ...
    * `:require_console_user` — mounts current_scope, halts unless role is admin/owner.
    * `:require_authenticated_user` — mounts current_scope, halts unless logged in.
    * `:mount_current_scope` — mounts current_scope with no auth requirement.
  """
  def on_mount(:require_console_user, _params, session, socket) do
